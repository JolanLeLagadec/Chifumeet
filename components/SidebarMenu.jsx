...
export default function SidebarMenu() {
    ...
    const sidebarClass = 'w-full transition transform ease-in-out duration-300 absolute min-h-screen bg-background '
    ...
    <div className={`${sidebarMenu.isOpen ? 'translate-x-0' : '-translate-x-full'}  ${sidebarClass} `}>
        <div className='min-h-screen flex mt-32 px-12 '>   
            <ul className='flex flex-col text-4xl space-y-12 mr-24'>
                ...
                <li className='cursor-pointer hover:opacity-60'>Déconnexion</li>
            </ul>
            <ModeToggle />
            <div className={`${sidebarMenu.isOpen ? 'h-[30rem]' : 'h-0'} ml-24 w-1  bg-primary-game transition-height ease-in-out duration-700 `}></div>
            </div>  
    </div>
  )
}
